...
# Next, let's convert the categorical features into numerical values so they can be used as model imputs.
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y)):
    ...
    test_pred_proba_mean = model.predict_proba(test)[:, 1]
...
submission.to_csv(OUTPUT_DIR / "submission_random_forest.csv", index=False)
